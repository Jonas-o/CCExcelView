...
- (void)resetCellContentViewSize {
    lockScrollView.frame = rectZP([self lockScrollViewContentWidth], self.bounds.size.height);
    lockScrollView.contentSize = lockScrollView.bounds.size;
    
    CGFloat rightWidth = [self rightScrollViewContentWidth];
    farrightLockScrollView.frame = rect(self.bounds.size.width-rightWidth, 0, rightWidth, self.bounds.size.height);
    ...
    contentScrollView.frame = rect(lockScrollView.bounds.size.width, 0, self.bounds.size.width - lockScrollView.frame.size.width - farrightLockScrollView.bounds.size.width, self.bounds.size.height);
    contentScrollView.contentSize = size(MAX(contentScrollView.bounds.size.width, contentWidth), self.bounds.size.height);
}
...
- (void)layoutSubviews {
    ...
    line.frame = rect(0, self.bounds.size.height - 1, self.bounds.size.width, 1);
}
...
- (void)setLockItems:(NSArray *)lockItems scrollItems:(NSArray *)scrollItems rightLockItems:(NSArray *)rightLockItems {
    ...
    for (int i = 0; i < lockItems.count; i++) {
        ...
        lockItem.frame = rectFromSize(currentOffsetX, 0, lockItem.bounds.size);
        currentOffsetX += lockItem.bounds.size.width;
        [lockScrollView addSubview:lockItem];
    }
    contentScrollView.frame = rect(currentOffsetX, 0, 0, 0);
    currentOffsetX = 0;
    for (int i = 0; i < scrollItems.count; i++) {
        ...
        scrollItem.frame = rectFromSize(currentOffsetX, 0, scrollItem.bounds.size);
        [contentScrollView addSubview:scrollItem];
        currentOffsetX += scrollItem.bounds.size.width;
    }
    contentScrollView.contentSize = size(currentOffsetX, 0);
    ...
    farrightLockScrollView.frame = rectZP(currentOffsetX, 0);
